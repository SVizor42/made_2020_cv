...
class ThousandLandmarksDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.albumentations is not None:
            temp = albu.Compose(self.albumentations, p=1,
                                  keypoint_params=albu.KeypointParams(format='xy', remove_invisible=False))(image=sample['image'],
                                                                                    keypoints=sample['landmarks'])
            sample['image'] = np.array(temp['image'])
            sample['landmarks'] = torch.as_tensor(np.array(temp['keypoints']))
